...
def Fase2(pygame, relogio, tela, superficie, desenhar_rede, cobra, vida, v, fonte, inimigo_main):
  ...
  inimigos = [inimigo, inimigo2, inimigo3, inimigo4, inimigo_movel1, inimigo_movel2]
  while True:
    ...
    #declaração da direção de movimento dos inimigos móveis
    inimigo_movel1.movel(v.CIMA)
    inimigo_movel2.movel(v.ESQUERDA)

    #pontuação para o portal
    if cobra.pontuacao >= 5:
        #abre o portal no centro da tela
        portal = Portal()
    ...
    # Checar se a cobra pegou a maçã
